...
template <typename T>
void UniformCylindricalFlatEndcap::operator()(
...
template <typename T>
void UniformCylindricalFlatEndcap::jacobian(
    const gsl::not_null<
  ...
  get<2, 0>(*result) = 0.5 * radius_one_ *
                       ...
                       (1.0 - zbar);
  get<1, 1>(*result) = get<2, 0>(*result) + 0.5 * radius_two_ * (1.0 + zbar);
  get<2, 1>(*result) = theta_max_one_ * get<2, 0>(*result);
  get<1, 2>(*result) =
      0.5 *
      (radius_two_ - radius_one_ * cylindrical_endcap_helpers::sin_ax_over_x(
                                       get<2, 2>(*result), theta_max_one_));
  get<1, 0>(*result) = 0.5 * radius_one_ *
...
#define INSTANTIATE_NOT_NULL(_, data)                                     \
  template void UniformCylindricalFlatEndcap::operator()(                 \
...
#define INSTANTIATE_JACOBIAN_NOT_NULL(_, data)                                \
  template void UniformCylindricalFlatEndcap::jacobian(                       \
      gsl::not_null<                                                          \
