...
DataType IsentropicVortex<Dim>::function_of_z(const DataType& z) const
    noexcept {
  return sin(z);
}
...
template <size_t Dim>
template <typename DataType>
DataType IsentropicVortex<Dim>::dz_function_of_z(const DataType& z) const
    noexcept {
  return cos(z);
}
...
IsentropicVortex<Dim>::variables(
  ...
  if (Dim == 3) {
    get<Dim - 1>(velocity) += perturbation_amplitude_ * function_of_z(vars.z);
  }
  return velocity;
}
...
#define INSTANTIATE_MEMBERS(_, data)                                        \
  template struct IsentropicVortex<DIM(data)>::IntermediateVariables<DTYPE( \
      data)>;                                                               \
  template DTYPE(data)                                                      \
      IsentropicVortex<DIM(data)>::function_of_z(const DTYPE(data) & z)     \
          const noexcept;                                                   \
  template DTYPE(data)                                                      \
      IsentropicVortex<DIM(data)>::dz_function_of_z(const DTYPE(data) & z)  \
          const noexcept;
