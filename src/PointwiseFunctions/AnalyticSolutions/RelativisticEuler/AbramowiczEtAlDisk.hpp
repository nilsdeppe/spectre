 ...
 *
 * The AJS solution to the 3D relativistic Euler system
 * \cite abramowicz1978aa, representing the isentropic flow of a thick fluid
 * disk orbiting a Kerr black hole. In Boyer-Lindquist coordinates
...
class AbramowiczEtAlDisk {
 ...
 public:
  ...
  AbramowiczEtAlDisk() = default;
  AbramowiczEtAlDisk(const AbramowiczEtAlDisk& /*rhs*/) = delete;
  AbramowiczEtAlDisk& operator=(const AbramowiczEtAlDisk& /*rhs*/) = delete;
  AbramowiczEtAlDisk(AbramowiczEtAlDisk&& /*rhs*/) noexcept = default;
  AbramowiczEtAlDisk& operator=(AbramowiczEtAlDisk&& /*rhs*/) noexcept =
      default;
  ~AbramowiczEtAlDisk() = default;

  AbramowiczEtAlDisk(double bh_mass, double bh_dimless_spin,
                     double inner_edge_radius, double max_pressure_radius,
                     double polytropic_constant,
                     double polytropic_exponent) noexcept;

  // Pointwise metric components in Kerr coordinates needed to setup disk.
  double g_tt(double r_squared, double sin_theta_squared) const noexcept;
  double g_tphi(double r_squared, double sin_theta_squared) const noexcept;
  double g_phiphi(double r_squared, double sin_theta_squared) const noexcept;

  double angular_velocity(double r_squared, double sin_theta_squared) const
      noexcept;

  double potential(double r_squared, double sin_theta_squared) const noexcept;
 ...
 protected:
  ...
  friend bool operator==(const AbramowiczEtAlDisk& lhs,
