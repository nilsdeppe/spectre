...
DataVector GridCenters::impl(const DataVector& fot_positions_dv,
  ...
  tnsr::I<DataVector, 3, Frame::Grid> fot_positions_tnsr{2_st, 0.0};
  for (size_t i = 0; i < 3; i++) {
    fot_positions_tnsr.get(i)[0] = fot_positions_dv[i];
    fot_positions_tnsr.get(i)[1] = fot_positions_dv[3 + i];
  }
