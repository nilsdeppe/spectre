...
bool FixToAtmosphere<Dim>::apply_kappa_limit(
  ...
  const double min_temperature = equation_of_state.temperature_lower_bound();
  ...
  const auto impl = [&get_pressure, &min_temperature, &temp,
    ...
    const double p_max = p_temp * local_kappa_max;
    if (p_temp > p_max) {
  ...
  if (density < opts.density_lower_bound) {
    ...
    const double kappa_max =
        1.0 + opts.epsilon_kappa_max *
                  (opts.density_upper_bound > opts.epsilon_kappa_plus_minus *
                                                  opts.density_lower_bound
                       ? ((density - opts.density_lower_bound) /
...
bool FixToAtmosphere<Dim>::KappaLimitingOptions::operator==(
  ...
  return density_lower_bound == rhs.density_lower_bound and
         ...
         epsilon_kappa_plus_minus == rhs.epsilon_kappa_plus_minus and
