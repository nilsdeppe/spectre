...
void compute_conservatives_for_reconstruction(
  ...
  // Computes:
  // 1. W v^i
  // 2. Lorentz factor as sqrt(1 + Wv^i Wv^i\gamma_{ij})
  // 3. v^i = Wv^i / W
...
void reconstruct_prims_work(
  ...
  const size_t nhbr_num_pts =
      ghost_zone_size * subcell_mesh.extents().slice_away(0).product();
  size_t vars_in_nhbr_count = 0;
  tmpl::for_each<prim_tags_for_reconstruction>(
      [&element, &neighbor_data, nhbr_num_pts, &reconstruct,
       reconstructed_num_pts, volume_num_pts, &volume_prims,
       &vars_in_nhbr_count, &vars_on_lower_face, &vars_on_upper_face,
       &subcell_mesh](auto tag_v) noexcept {
        ...
        const gsl::span<const double> volume_vars =
            gsl::make_span((*volume_tensor_ptr)[0].data(),
                           number_of_variables * volume_num_pts);
        ...
        for (const auto& direction : Direction<3>::all_directions()) {
          const auto& nhbrs_in_direction = element.neighbors().at(direction);
          ASSERT(nhbrs_in_direction.size() == 1,
                 "Currently only support one neighbor in each direction, but "
                 "got "
                     << nhbrs_in_direction.size() << " in direction "
                     << direction);
          ghost_cell_vars[direction] = gsl::make_span(
              &neighbor_data
                   .at(std::pair{direction, *nhbrs_in_direction.begin()})
                   .data_for_reconstruction[vars_in_nhbr_count * nhbr_num_pts],
              number_of_variables * nhbr_num_pts);
        }
        ...
        vars_in_nhbr_count += number_of_variables;
      });
...
void reconstruct_fd_neighbor_work(
  ...
  Variables<prim_tags_for_reconstruction> nhbr_prims{
      ghost_data_extents.product()};
  {
    ...
    const auto& nhbr_data = neighbor_data.at(mortar_id);
    std::copy(nhbr_data.data_for_reconstruction.begin(),
              std::next(nhbr_data.data_for_reconstruction.begin(),
                        static_cast<std::ptrdiff_t>(
                            nhbr_prims.number_of_independent_components *
                            ghost_data_extents.product())),
              nhbr_prims.data());
  }

  tmpl::for_each<prim_tags_for_reconstruction>(
      [&direction_to_reconstruct, &ghost_data_extents, &nhbr_prims,
       ...
       &subcell_volume_prims, &vars_on_face](auto tag_v) noexcept {
        ...
        const auto& tensor_nhbr = get<tag>(nhbr_prims);
        ...
        if (direction_to_reconstruct.side() == Side::Upper) {
          for (size_t tensor_index = 0; tensor_index < tensor_on_face.size();
            ...
            reconstruct_upper_neighbor(
                ...
                (*volume_tensor_ptr)[tensor_index], tensor_nhbr[tensor_index],
