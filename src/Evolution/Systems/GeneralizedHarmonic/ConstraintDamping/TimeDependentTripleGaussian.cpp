...
void TimeDependentTripleGaussian::apply_call_operator(
  ...
  if (this->movement_method_ == MovementMethods::ExpansionFactor) {
    ...
    // TODO: check that center one and center two are object A and B
    // respectively.
    ...
    ASSERT(centers.size() == 6,
           ...
           "size components, not "
               << functions_of_time.at(function_of_time_for_scaling_)
                      ->func(time)[0]
                      .size());
    std::array center_1{centers[0], centers[1], centers[2]};
    std::array center_2{centers[3], centers[4], centers[5]};
