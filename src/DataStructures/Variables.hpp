...
class Variables<tmpl::list<Tags...>> {
 ...
 public:
  ...
  /// Construct a non-owning Variables that points to `start`. The allocation
  /// size pointed to be start must be
  /// `number_of_grid_points * Variables::number_of_independent_components`
  Variables(pointer start, size_t number_of_grid_points);
...
Variables<tmpl::list<Tags...>>::Variables(const pointer start,
                                          const size_t number_of_grid_points) {
  set_data_ref(start, number_of_grid_points * number_of_independent_components);
}
