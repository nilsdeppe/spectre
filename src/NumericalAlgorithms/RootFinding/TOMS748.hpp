...
template <bool AssumeFinite, typename T, typename Arch>
simd::batch<T, Arch> quadratic_interpolate(
    const simd::batch<T, Arch>& a, const simd::batch<T, Arch>& b,
    const simd::batch<T, Arch>& d, const simd::batch<T, Arch>& fa,
    const simd::batch<T, Arch>& fb, const simd::batch<T, Arch>& fd,
    const simd::batch_bool<T, Arch>& incomplete_mask, const unsigned count);
template <bool AssumeFinite, typename T, typename Arch>
simd::batch<T, Arch> cubic_interpolate(
    ...
    const simd::batch_bool<T, Arch>& incomplete_mask);
...
simd::batch<T, Arch> quadratic_interpolate(
  ...
  // code quite a bit.
  //
  // TODO: check the zero-cases are handled correct with simd.
  simd::batch<T, Arch> c =
      AssumeFinite
          ? simd::select(A * fa > static_cast<T>(0), a, b)
          : simd::select(simd::sign(A) * simd::sign(fa) > static_cast<T>(0), a,
                         b);
...
template <bool AssumeFinite, typename F, typename T, typename Arch>
void bracket(F f, simd::batch<T, Arch>& a, simd::batch<T, Arch>& b,
  ...
  if (const auto mask = fc_is_zero_mask and incomplete_mask;
    ...
    fa = simd::select(mask, simd::batch<T, Arch>(static_cast<T>(0)),
                      fa);
    d = simd::select(mask, simd::batch<T, Arch>(static_cast<T>(0)),
                     d);
    fd = simd::select(mask, simd::batch<T, Arch>(static_cast<T>(0)),
                      fd);
  ...
  const auto sign_mask =
      AssumeFinite ? (fa * fc < static_cast<T>(0)) and (fa != static_cast<T>(0))
                   : (sign(fa) * sign(fc) < static_cast<T>(0)) and
                         (fa != static_cast<T>(0));
  const auto mask_if =
...
template <bool AssumFinite, class F, class T, class Arch, class Tol,
...
std::pair<simd::batch<T, Arch>, simd::batch<T, Arch>> toms748_solve(
  ...
  // TODO: handle zero-case correctly when using simd::sign? Need to check if
  // simd::sign(0) == 0
  //
  // Note: unlike Boost, we assume a*fb doesn't overflow. This speeds up the
  // code quite a bit.
  if (UNLIKELY(
          simd::any((AssumeFinite ? a * fb : simd::sign(a) * simd::sign(fb)) >
                        static_cast<T>(0) and
  ...
  auto completed_fa = simd::select(completion_mask, fa, simd_nan);
  auto completed_fb = simd::select(completion_mask, fb, simd_nan);
  ...
  if (simd::any(fa != static_cast<T>(0))) {
    ...
    if (count and not update_completed()) {
      ...
      c = toms748_detail::quadratic_interpolate<AssumeFinite>(
          a, b, d, fa, fb, fd, incomplete_mansk, 2);
      e = d;
      fe = fd;
      toms748_detail::bracket(f, a, b, c, fa, fb, d, fd, incomplete_mask);
      --count;
...
template <bool AssumeFinite, typename Function, typename T, typename Arch>
