...
const Matrix& HollowCylinder<TagList>::exponential_filter_matrix(
  ...
  const auto it = cache.find(extent);
  if (it != cache.end()) {
    return it->second;
  }
  return cache
...
const Matrix& HollowCylinder<TagList>::angular_filter_matrix(
  ...
  const size_t extent = mesh_1d.extents(0);
  const auto it = cached_angular_filters_.find(extent);
  if (it != cached_angular_filters_.end()) {
    return it->second;
  }
  Matrix combined{};
  ...
  return cached_angular_filters_.emplace(extent, std::move(combined))
      .first->second;
}
...
void HollowCylinder<TagList>::apply_in_volume(
  ...
  const Matrix& radial = exponential_filter_matrix(
      radial_half_power_, mesh.slice_through(0), cached_radial_filters_);
  const Matrix& angular = angular_filter_matrix(mesh.slice_through(1));
  const Matrix& z = exponential_filter_matrix(
      z_half_power_, mesh.slice_through(2), cached_z_filters_);
...
void HollowCylinder<TagList>::apply_on_boundary(
  ...
  //   (Legendre, Legendre) -> (radial, z)   [angular face]
  ...
  if (fourier_0 and not fourier_1) {
    ...
    filter[1] = std::cref(exponential_filter_matrix(
        z_half_power_, mesh.slice_through(1), cached_z_filters_));
    ...
    ERROR(
        "HollowCylinder filter called on a face mesh with an unexpected basis "
        "combination. Got basis ("
        << mesh.basis(0) << ", " << mesh.basis(1)
        << "); a hollow-cylinder face must have exactly one Fourier "
           "direction or none.");
  }
  *vars = apply_matrices(filter, *vars, mesh.extents());
}
