 ...
 * \f$z\f$ direction (logical dimension 2). This filter is therefore distinct
 * from `Filters::Hypercube` (which applies a single `HalfPower` to every
 * direction) in that it exposes independent half-powers for each of the three
 * directions, together with a hard top-mode cutoff in the angular direction
 * analogous to `Filters::SphericalShell`'s `NumModesToKill`.
 *
 ...
 * `Hypercube` and `SphericalShell`. Because the filter operates entirely in
 * logical space using `Spectral::filtering` utilities, it requires no
 * system-specific instantiation and is registered in `Filters::all_filters`.
 */
...
class HollowCylinder : public Filter<3, TagList> {
 ...
 private:
  ...
  const Matrix& exponential_filter_matrix(
      std::optional<size_t> half_power, const Mesh<1>& mesh_1d,
      std::unordered_map<size_t, Matrix>& cache) const;
  ...
  mutable std::unordered_map<size_t, Matrix> cached_radial_filters_{};
  // NOLINTNEXTLINE(spectre-mutable)
  mutable std::unordered_map<size_t, Matrix> cached_angular_filters_{};
  // NOLINTNEXTLINE(spectre-mutable)
