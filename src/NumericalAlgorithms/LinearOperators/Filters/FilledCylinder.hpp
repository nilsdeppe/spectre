 ...
 * - an angular face (ZernikeB2 with `GaussRadauUpper` quadrature in the radial
 *   direction, Legendre/Chebyshev in \f$z\f$) is filtered only in \f$z\f$,
 *   because a lone one-dimensional radial Zernike slice has no filter engine.
 *
 * Because the filter operates entirely in logical space using
 * `Spectral::filtering` utilities, it requires no system-specific instantiation
 * and is registered in `Filters::all_filters`.
 */
...
class FilledCylinder : public Filter<3, TagList> {
 ...
 private:
  ...
  const Matrix& z_filter_matrix(std::optional<size_t> half_power,
                                const Mesh<1>& mesh_1d) const;

  // Returns the cached angular (Fourier) filter matrix used on the mantle face,
  // combining the optional exponential roll-off and the optional top-mode
  // cutoff. The matrix is built from a Fourier/Equiangular mesh with the same
  // extent as `num_angular_points` so that it can be applied to the equiangular
  // ZernikeB2-angular nodal data. Returns an empty (identity) matrix when
  // neither is requested.
  const Matrix& mantle_angular_filter_matrix(size_t num_angular_points) const;
  ...
  // Caches keyed by 1-D extent. The empty matrix is returned (and not cached)
  // when a direction is not filtered.
  // NOLINTNEXTLINE(spectre-mutable)
  mutable std::unordered_map<size_t, Matrix> cached_z_filters_{};
  // NOLINTNEXTLINE(spectre-mutable)
  mutable std::unordered_map<size_t, Matrix> cached_mantle_angular_filters_{};
