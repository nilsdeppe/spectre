 ...
 * zero, with the \f$m=0\f$ mode always retained. If `half_power` is `None` and
 * `num_modes_to_kill` is zero this function is a no-op.
 *
 ...
 * by all exponential roll-offs. Any direction whose half-power is `None` (and,
 * for the disk, with `num_modes_to_kill` zero) is left untouched.
 *
 * \see exponential_filter()
 */
template <typename TagsList>
