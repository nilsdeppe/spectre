...
def boyer_lindquist_gtt(r_sqrd, sin_theta_sqrd, m, a):
    return 2.0 * m * np.sqrt(r_sqrd) / sigma(r_sqrd, sin_theta_sqrd, a) - 1.0
...
def boyer_lindquist_gff(r_sqrd, sin_theta_sqrd, m, a):
    return (((r_sqrd + a**2)**2 - delta(r_sqrd, m, a) *
             sin_theta_sqrd * a**2) * sin_theta_sqrd / sigma(
                 r_sqrd, sin_theta_sqrd, a))
...
def angular_momentum(m, a, rmax):
    sqrt_m = np.sqrt(m)
    return sqrt_m * ((rmax**2 - 2.0 * a * np.sqrt(m * rmax) + a**2) /
                     ((rmax - 2.0 * m) * np.sqrt(rmax) + a * sqrt_m))


def angular_velocity(angular_momentum, r_sqrd, sin_theta_sqrd, m, a):
    gtf = boyer_lindquist_gtf(r_sqrd, sin_theta_sqrd, m, a)
    return -((gtf + angular_momentum *
              boyer_lindquist_gtt(r_sqrd, sin_theta_sqrd, m, a)) /
             (boyer_lindquist_gff(r_sqrd, sin_theta_sqrd, m, a) +
              angular_momentum * gtf))


def potential(angular_momentum, r_sqrd, sin_theta_sqrd, m, a):
    return 0.5 * np.log(delta(r_sqrd, m, a) * sin_theta_sqrd /
                        (boyer_lindquist_gff(r_sqrd, sin_theta_sqrd, m, a) +
                         2.0 * angular_momentum *
