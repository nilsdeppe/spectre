...
namespace {
struct AbramowiczEtAlDiskProxy
    : RelativisticEuler::Solutions::AbramowiczEtAlDisk {
  using RelativisticEuler::Solutions::AbramowiczEtAlDisk::AbramowiczEtAlDisk;
...
void test_create_from_options() noexcept {
  const auto disk =
      test_creation<RelativisticEuler::Solutions::AbramowiczEtAlDisk>(
          "  BhMass: 1.0\n"
          "  BhDimlessSpin: 0.23\n"
          "  InnerEdgeRadius: 6.0\n"
  ...
  CHECK(disk == RelativisticEuler::Solutions::AbramowiczEtAlDisk(
                    1.0, 0.23, 6.0, 12.0, 0.001, 1.4));
}

void test_move() noexcept {
  RelativisticEuler::Solutions::AbramowiczEtAlDisk disk(3.45, 0.23, 4.8, 8.6,
                                                        0.02, 1.5);
  RelativisticEuler::Solutions::AbramowiczEtAlDisk disk_copy(3.45, 0.23, 4.8,
                                                             8.6, 0.02, 1.5);
  test_move_semantics(std::move(disk), disk_copy);  //  NOLINT
}

void test_serialize() noexcept {
  RelativisticEuler::Solutions::AbramowiczEtAlDisk disk(4.21, 0.65, 6.0, 12.0,
                                                        0.001, 1.4);
...
void test_variables(const DataType& used_for_size) noexcept {
  ...
  AbramowiczEtAlDiskProxy disk(bh_mass, bh_dimless_spin, inner_edge_radius,
  ...
  pypp::check_with_random_values<
      1, AbramowiczEtAlDiskProxy::hydro_variables_tags<DataType>>(
      &AbramowiczEtAlDiskProxy::hydro_variables<DataType>, disk,
      "AbramowiczEtAlDisk",
  ...
  pypp::check_with_random_values<
      1, AbramowiczEtAlDiskProxy::grmhd_variables_tags<DataType>>(
      &AbramowiczEtAlDiskProxy::grmhd_variables<DataType>, disk,
      "AbramowiczEtAlDisk",
      {"rest_mass_density", "spatial_velocity", "specific_internal_energy",
...
void test_solution() noexcept {
  RelativisticEuler::Solutions::AbramowiczEtAlDisk solution(
      1.0, 0.9, 2.6116, 4.62, 0.01, 4.0 / 3.0);
  const std::array<double, 3> x{{4.7, 0.03, -0.04}};
  ...
  verify_grmhd_solution(solution, domain.blocks()[0], mesh, 1.e-8, 1.234,
                        1.e-4);
}
}  // namespace

SPECTRE_TEST_CASE("Unit.PointwiseFunctions.AnalyticSolutions.RelEuler.AJSDisk",
...
struct Disk {
  using type = RelativisticEuler::Solutions::AbramowiczEtAlDisk;
...
// bound of 0]]
SPECTRE_TEST_CASE(
    "Unit.PointwiseFunctions.AnalyticSolutions.RelEuler.AJSDiskBHMassOpt",
...
// lower bound of 0]]
SPECTRE_TEST_CASE(
    "Unit.PointwiseFunctions.AnalyticSolutions.RelEuler.AJSDiskBHSpinLowerOpt",
    "[PointwiseFunctions][Unit]") {
  ERROR_TEST();
  Options<tmpl::list<Disk>> test_options("");
  test_options.parse(
...
// upper bound of 1.]]
SPECTRE_TEST_CASE(
    "Unit.PointwiseFunctions.AnalyticSolutions.RelEuler.AJSDiskBHSpinUpperOpt",
    "[PointwiseFunctions][Unit]") {
  ERROR_TEST();
  Options<tmpl::list<Disk>> test_options("");
  test_options.parse(
...
// lower bound of 0]]
SPECTRE_TEST_CASE(
    "Unit.PointwiseFunctions.AnalyticSolutions.RelEuler.AJSDiskPolytConstOpt",
    "[PointwiseFunctions][Unit]") {
  ERROR_TEST();
  Options<tmpl::list<Disk>> test_options("");
  test_options.parse(
...
// lower bound of 1]]
SPECTRE_TEST_CASE(
    "Unit.PointwiseFunctions.AnalyticSolutions.RelEuler.AJSDiskPolytExpOpt",
    "[PointwiseFunctions][Unit]") {
  ERROR_TEST();
  Options<tmpl::list<Disk>> test_options("");
  test_options.parse(
