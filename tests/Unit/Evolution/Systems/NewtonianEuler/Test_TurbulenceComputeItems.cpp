...
void test_compute_items_in_databox(
  ...
  TestHelpers::db::test_compute_tag<
      NewtonianEuler::Tags::MachCompute<DataType, Dim>>("Mach");
  TestHelpers::db::test_compute_tag<
  ...
  const auto box = db::create<
      ...
      db::AddComputeTags<
          ...
          NewtonianEuler::Tags::MachCompute<DataType, Dim>,
          NewtonianEuler::Tags::SpecificKineticEnergyCompute<DataType, Dim>,
          NewtonianEuler::Tags::KineticEnergyDensityCompute<DataType, Dim>,
          NewtonianEuler::Tags::InternalEnergyDensityCompute<DataType>,
  ...
  const auto expected_mach =
      NewtonianEuler::mach(velocity, expected_sound_speed);
  ...
  CHECK(db::get<NewtonianEuler::Tags::Mach<DataType>>(box) == expected_mach);
...
void test_compute_items(const DataType& used_for_size) noexcept {
  ...
  CHECK(Scalar<DataType>{0.5 * get(dot_product(velocity, velocity))} ==
        NewtonianEuler::specific_kinetic_energy(velocity));
  CHECK(Scalar<DataType>{0.5 * get(mass_density) *
                         get(dot_product(velocity, velocity))} ==
        NewtonianEuler::kinetic_energy_density(mass_density, velocity));
  ...
  // CHECK implementation of Mach here
