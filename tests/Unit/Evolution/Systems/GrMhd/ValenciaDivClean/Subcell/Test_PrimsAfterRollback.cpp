...
void test(const gsl::not_null<std::mt19937*> gen,
  ...
  // Compute the conservatives on the DG grid by first computing the primitives
  // on the DG grid, then compute the conservatives from the primitives.
  ...
  if (did_rollback) {
    ...
    get(get<hydro::Tags::Pressure<DataVector>>(expected_subcell_prims)) =
        evolution::dg::subcell::fd::project(
            get(db::get<hydro::Tags::Pressure<DataVector>>(box)), dg_mesh,
            subcell_mesh.extents());
