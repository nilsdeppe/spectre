...
SPECTRE_TEST_CASE(
  ...
  for (const ::fd::DerivativeOrder fd_do :
  ...
  {
    const auto five_pts_data = test(5, DO::OneHigherThanRecons);
    const auto six_pts_data = test(6, DO::OneHigherThanRecons);
    for (size_t i = 0; i < five_pts_data.size(); ++i) {
      CAPTURE(i);
      CHECK(gsl::at(six_pts_data, i) < gsl::at(five_pts_data, i));
  ...
  {
    const auto five_pts_data = test(5, DO::OneHigherThanReconsButFiveToFour);
    const auto six_pts_data = test(6, DO::OneHigherThanReconsButFiveToFour);
    for (size_t i = 0; i < five_pts_data.size(); ++i) {
      CAPTURE(i);
      CHECK(gsl::at(six_pts_data, i) < gsl::at(five_pts_data, i));
      CHECK(gsl::at(five_pts_data, i) < gsl::at(second_order_error_5, i));
